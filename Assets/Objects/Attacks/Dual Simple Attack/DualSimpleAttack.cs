 ...
 * @date 10/05/2021
 * An attack that spawns two simple projectiles at the same time. Based on the single simple attack script
 */
public sealed class DualSimpleAttack : AttackBase
{
	...
	private SimpleMagicalProjectile spawnMagical(System.Random rng, Vector3 player_pos, Quaternion player_rot, Health player_health, ScoreSystem player_score)
	{
		...
		SimpleMagicalProjectile proj = GameObject.Instantiate(this.MAGIC_PROJ, projpos, Quaternion.identity).GetComponent<SimpleMagicalProjectile>();
	...
	{
		projpos.x *= -1;
		SimplePhysicalProjectile proj = GameObject.Instantiate(this.PHYSICS_PROJ, projpos, Quaternion.identity).GetComponent<SimplePhysicalProjectile>();
		proj.SPEED = this.randSpeed(rng);
		proj.TARGET = player_pos;
		proj.PLAYER_HEALTH = player_health;
		proj.PLAYER_SCORE = player_score;
